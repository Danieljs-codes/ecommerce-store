...
const SidebarItem = ({
      ...
      className={composeRenderProps(
        ...
        (className, { isPressed, isFocusVisible, isHovered, isDisabled }) =>
          twMerge([
            ...
            isFocusVisible &&
              'inset-ring inset-ring-ring outline-hidden ring-2 ring-ring/20',
            (isPressed || isHovered) &&
  ...
  return isCollapsed && tooltip ? (
    <Tooltip delay={0}>
      {link}
      <Tooltip.Content
        className="**:data-[slot=icon]:hidden **:data-[slot=sidebar-label-mask]:hidden text-xs"
        intent="default"
        showArrow={false}
        placement="right"
      >
...
// const SidebarLink = createLink(
//   ({ className, ref, ...props }: SidebarLinkProps) => {
//     const { state, isMobile } = useSidebar()
//     const collapsed = state === 'collapsed' && !isMobile
//     return (
//       <Link
