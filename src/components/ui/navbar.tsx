  ...
  return (
    ...
    >
      ...
      >
        <div
          data-navbar="content"
          className="mx-auto w-full max-w-(--breakpoint-xl) items-center py-2.5 md:flex"
        >
          {children}
        </div>
      </div>
...
const NavbarItem = createLink(
  ({ className, isCurrent, ...props }: NavbarItemProps) => {
    return (
      <Link
            ...
            {(isCurrent || values.isCurrent) && (
              <span
                ...
                className={twJoin(
                  ...
                  'md:-bottom-[--spacing(2.8)] md:group-data-[navbar=inset]/navbar-intent:-bottom-[--spacing(3.1)] md:inset-x-2 md:h-(--gutter)',
                )}
              />
            )}
          </>
